...
exports.createFarmer = catchAsyncErrors(async (req, res) => {
  const { name, phone_number, password, district, state, crop_name, language } =
    req.body;

  const PHno_Validation = await Farmer.findOne({ phone_number });
...
exports.FarmerLogin = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    if (!FarmerDetails) {
      ...
      return res
        .status(404)
        .json({ success: false, error: "Invalid phone_number or password." });
    }
    // Compare passwords
    ...
    try {
      ...
      if (StartupsAvai.length === 0) {
        ...
        return res.status(404).json({
          StartupRetrievalsuccess: false,
          error: "No Startups Available in the farmer's district.",
        });
      }
      ...
      res.status(200).json({
        success: true,
        StartupRetrievalsuccess: true,
        Tokensuccess: true,
        message: "Startup Details for farmer",
        StartupsAvai,
      });
    } catch (error) {
